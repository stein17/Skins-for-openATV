...
#  DEUTSCH / GERMAN
# =============================================================================
#  GradientWQHD Config for Enigma2 Skins
#
...
config.plugins.GradientWQHD.refreshInterval = ConfigNumber(default=10)
config.plugins.GradientWQHD.woeid = ConfigNumber(default=638242)


# PrimeTime (default 20:15)
config.plugins.GradientWQHD.primeTimeHour = ConfigSelection(
    default="20",
    choices=[(str(i), "%02d" % i) for i in range(0, 24)]
)
config.plugins.GradientWQHD.primeTimeMinute = ConfigSelection(
    default="15",
    choices=[(str(i), "%02d" % i) for i in range(0, 60)]
)

config.plugins.GradientWQHD.tempUnit = ConfigSelection(default="Celsius", choices=[
                                ("Celsius", _("Celsius")),
...
#
# ---------------------------------------------------------------------
config.plugins.GradientWQHD.posterXPath = ConfigSelection(
    default="/media/hdd",
    choices=[
...
try:
    if getattr(config.plugins.GradientWQHD, "posterXPath", None) and config.plugins.GradientWQHD.posterXPath.value == "AUTO":
        config.plugins.GradientWQHD.posterXPath.value = "/media/hdd"
        config.plugins.GradientWQHD.posterXPath.save()
        configfile.save()
except Exception:
    pass
# ------------------------------------------
...
def _gradientwqhd_channel_selection_choices():
    ...
    try:
        ...
        print("[GradientWQHD] Senderlisten-Auswahl konnte nicht gelesen werden: %s" % error)
        return None, None


def _gradientwqhd_skinchange(session=None, **kwargs):
    ...
    try:
        ...
        print("[GradientWQHD] Senderlisten-Templates konnten nicht neu geladen werden: %s" % error)

    screen_choices, template_choices = _gradientwqhd_channel_selection_choices()
    ...
    configfile.save()
    print("[GradientWQHD] Senderlisten-Auswahl synchronisiert: %s / %s" % (screen_config.value, template_config.value))


def Plugins(**kwargs):
    return [PluginDescriptor(name=_("GradientWQHD  Configtool"), description=_("Personalize your GradientWQHD (Skin by stein17)"), where=[PluginDescriptor.WHERE_PLUGINMENU],
    icon="plugin.png", fnc=main),
    PluginDescriptor(where=[PluginDescriptor.WHERE_SESSIONSTART], fnc=_gradientwqhd_sessionstart),
    PluginDescriptor(where=[PluginDescriptor.WHERE_SKINCHANGE], fnc=_gradientwqhd_skinchange)]


def main(session, **kwargs):
    if config.skin.primary_skin.value == "GradientWQHD/skin.xml":
        session.open(GradientWQHD_Config)
    else:
        AddPopup(_('Please activate GradientWQHD Skin before run the Config Plugin'), type=MessageBox.TYPE_ERROR, timeout=10)
        return []
...
class WeatherLocationChoiceList(Screen):
    skin = """
            <screen name="WeatherLocationChoiceList" position="center,center" size="1707,960" title="Location list" >
                    <widget source="Title" render="Label" position="93,63" size="1267,57" font="Regular;47" transparent="1" />
                    <widget name="choicelist" position="93,153" size="933,640" font="Regular;27" itemHeight="33" scrollbarMode="showOnDemand" scrollbarWidth="8" transparent="1" />
                    <eLabel position=" 73,900" size="387, 7" zPosition="-10" backgroundColor="red" />
                    <eLabel position="467,900" size="387, 7" zPosition="-10" backgroundColor="green" />
                    <eLabel position="860,900" size="387, 7" zPosition="-10" backgroundColor="yellow" />
                    <eLabel position="1253,900" size="387, 7" zPosition="-10" backgroundColor="blue" />
                    <widget name="key_red" position="93,847" size="347,33" zPosition="1" font="Regular;27" halign="left" foregroundColor="foreground" transparent="1" />
                    <widget name="key_green" position="487,847" size="347,33" zPosition="1" font="Regular;27" halign="left" foregroundColor="foreground" transparent="1" />
            </screen>
            """
...
class GradientWQHD_Config(Screen, ConfigListScreen):

    skin = """
	<screen name="GradientWQHD_Config" position="center,center" size="2560,1440" title="GradientWQHD Setup" flags="wfNoBorder" backgroundColor="transparent">
		<eLabel text="/ Save" position="520,1325" size="320,51" font="Gradient_Font;36" halign="center" backgroundColor="black" transparent="1" valign="center" />
		<widget name="hint_yellow" position="160,1040" size="1289,56" font="Gradient_Font; 45" halign="left" transparent="1" valign="center" foregroundColor="yellow" backgroundColor="background" />
		<widget name="hint_menu_weather" position="160,1113" size="1289,56" font="Gradient_Font; 45" halign="left" transparent="1" valign="center" foregroundColor="green" backgroundColor="background" />
		<widget name="hint_epg_autodb" position="160,1187" size="1289,96" font="Gradient_Font; 43" halign="left" transparent="1" valign="center" foregroundColor="green" backgroundColor="background" />
		<eLabel text="GradientWQHD Config" position="60,20" size="1480,80" font="Italic; 56" halign="left" valign="center" transparent="1" foregroundColor="gradient_foreground_selection" backgroundColor="background" textBorderColor="black" textBorderWidth="1" zPosition="1" />		
		<widget name="config" position="40,120" size="1400,880" itemHeight="59" font="Gradient_Font;40" backgroundColor="gradient_background" foregroundColor="gradient_foreground" itemCornerRadiusSelected="16" itemGradientSelected="gradient_BGLM,gradient_BGLL,gradient_BGLL,horizontal" foregroundColorSelected="gradient_foreground_selection" scrollbarMode="showOnDemand" enableWrapAround="1" transparent="1" />
		<widget name="Picture" position="1467,133" size="1043,645" alphatest="on" zPosition="2" />
		<eLabel name="Line_Setup" position="40,1013" size="1400,3" zPosition="10" backgroundColor="gradient_BGLR,gradient_BGLM,gradient_BGLR,horizontal" />
        <ePixmap  name="QR" pixmap="icons/preview_with_QR-Code.png" position="1467,800" size="1043,480" zPosition="10" alphatest="blend"/>
		<ePixmap position="64,1116" size="80,48" zPosition="10" pixmap="buttons/menu.png" transparent="1" alphatest="blend" />
		<ePixmap position="2107,1325" size="80,48" zPosition="10" pixmap="buttons/key_epg.png" transparent="1" alphatest="blend" /> 
		<ePixmap pixmap="buttons/key_yellow.png" position="87,1048" size="40,40" alphatest="blend" transparent="1" />
		<ePixmap position="64,1213" size="80,48" zPosition="10" pixmap="buttons/key_epg.png" transparent="1" alphatest="blend" />
		<panel name="Fullscreen_TopPanel" />
		<panel position="612,196" size="440,76">
			<panel name="Template_Text_Buttons_M_O_E" />
		</panel>
        <panel position="40,196" size="440,76">
		<panel name="Template_Gradient_Config_Button_Red_Automatic_all" />
		<panel name="Template_Gradient_Config_Button_Green_Automatic_all" />
		<panel name="Template_Gradient_Config_Button_Yellow_Automatic_all" />
		<ePixmap pixmap="buttons/key_blue.png" position="1520,1133" size="40,40" alphatest="blend" zPosition="10" transparent="1" />
		<eLabel text="Help Settings Color" position="1567,1115" size="500,77" backgroundColor="ButtonBack" font="Gradient_Font;36" foregroundColor="ButtonText" halign="left" transparent="1" valign="center" zPosition="10" />
		<widget name="key_blue" position="980,1128" size="1,1" backgroundColor="ButtonBack" font="Gradient_Font;36" foregroundColor="ButtonText" zPosition="+1" valign="center" halign="left" objectTypes="key_blue,Button,Label" transparent="1" />
		<widget source="key_blue" render="Label" position="980,1128" size="1,1" backgroundColor="ButtonBack" font="Gradient_Font;36" foregroundColor="ButtonText" zPosition="+1" valign="center" halign="left" objectTypes="key_blue,StaticText" transparent="1" />
        </panel>
	</screen>
    """
    ...
    def openApiKeys(self):
        ...
        try:
            from .api_keys_config import GradientWQHD_APIKeysSetup
        except Exception as e:
            ...
            return
        self.session.open(GradientWQHD_APIKeysSetup)


    def openPrimeTime(self):
        try:
            self.session.open(GradientWQHD_PrimeTimeSetup)
        except Exception as e:
            self.session.open(MessageBox, tr(
    ...
    def getInitConfig(self):
        ...
        self.title = _("GradientWQHD Setup")
    ...
    def createConfigList(self):
        ...
        self.posterx_path_setup = getConfigListEntry(tr('PosterX storage path', 'PosterX Speicherpfad auswählen'), config.plugins.GradientWQHD.posterXPath)
    ...
    def keyYellow(self):
        if self.myAtileHD_active.value:
            self.session.openWithCallback(self.GradientWQHDScreenCB, GradientWQHDScreens)
        else:
    ...
    def select_weather_id_callback(self, res):
        if res and isInteger(res):
            print(res)
            config.plugins.GradientWQHD.woeid.value = int(res)
    ...
    def about(self):
        self.session.open(GradientWQHD_About)
...
class GradientWQHD_PrimeTimeSetup(Screen, ConfigListScreen):

    skin = """
    <screen name="GradientWQHD_PrimeTimeSetup" position="center,center" size="1707,613" title="PrimeTime Settings" flags="wfNoBorder" backgroundColor="transparent">
        ...
        <widget name="key_red" position="80,547" size="267,53" backgroundColor="ButtonBack" font="Gradient_Font;36" foregroundColor="ButtonText" valign="center" halign="left" transparent="1" />
        <widget name="key_green" position="400,547" size="267,53" backgroundColor="ButtonBack" font="Gradient_Font;36" foregroundColor="ButtonText" valign="center" halign="left" transparent="1" />
        <widget name="key_yellow" position="720,547" size="413,53" backgroundColor="ButtonBack" font="Gradient_Font;36" foregroundColor="ButtonText" valign="center" halign="left" transparent="1" />
        <widget name="key_blue" position="1187,547" size="267,53" backgroundColor="ButtonBack" font="Gradient_Font;36" foregroundColor="ButtonText" valign="center" halign="left" transparent="1" />
    </screen>
    """
    ...
    def __init__(self, session):
        ...
        self.list.append(getConfigListEntry(tr("Primetime: Hour", "Primetime: Stunde"), config.plugins.GradientWQHD.primeTimeHour))
        self.list.append(getConfigListEntry(tr("Primetime: Minutes", "Primetime: Minuten"), config.plugins.GradientWQHD.primeTimeMinute))
    ...
    def setDefault(self):
        config.plugins.GradientWQHD.primeTimeHour.value = "20"
        config.plugins.GradientWQHD.primeTimeMinute.value = "15"
        self["config"].l.setList(self.list)
    ...
    def save(self):
        ...
        try:
            h = int(config.plugins.GradientWQHD.primeTimeHour.value)
            m = int(config.plugins.GradientWQHD.primeTimeMinute.value)
...
class GradientWQHD_About(Screen):
...
class GradientWQHDScreens(Screen):

    skin = """
	<screen name="GradientWQHDScreens" position="center,center" size="2560,1440" title="GradientWQHD Setup" flags="wfNoBorder" backgroundColor="transparent">
		<eLabel text="Configure GradientWQHD-WQHD Skin" position="60,20" size="1480,80" font="Italic; 56" halign="left" valign="center" transparent="1" foregroundColor="gradient_foreground_selection" backgroundColor="background" textBorderColor="black" textBorderWidth="1" zPosition="1" />
		<widget source="menu" render="Listbox" position="40,133" size="1400,1140" backgroundColor="gradient_background" foregroundColor="gradient_foreground" itemCornerRadiusSelected="16" itemGradientSelected="gradient_BGLM,gradient_BGLL,gradient_BGLL,horizontal" foregroundColorSelected="gradient_foreground_selection" scrollbarMode="showOnDemand" enableWrapAround="1" transparent="1">
			<convert type="TemplatedMultiContent">{"template":
   [
    MultiContentEntryPixmapAlphaTest(pos = (11, 7), size = (60, 60), png = 2),
    MultiContentEntryText(pos = (80, 4), size = (1227, 60), font=0, flags = RT_HALIGN_LEFT|RT_VALIGN_CENTER, text = 1),
   ],
   "fonts": [gFont("Gradient_Font", 40),gFont("Gradient_Font", 32)],
   "itemHeight": 60
     }</convert>
		</widget>
		<ePixmap pixmap="preview/preview_background_color_skin_Gradient.png" position="1467,133" size="1043,645" zPosition="1" />
		<widget name="Picture" position="1467,133" size="1043,645" alphatest="on" zPosition="2" />
...
class PosterXSlugViewer(Screen):
    skin = """
        <screen name="PosterXSlugViewer" position="center,center" size="1600,933" title="PosterX Slugs">
            <widget name="title" position="13,13" size="1573,53" font="Regular;40" />
            <widget name="list" position="13,80" size="1573,760" font="Regular;27" itemHeight="33" scrollbarMode="showOnDemand" />
            <widget name="hint" position="13,853" size="1573,60" font="Regular;27" />
        </screen>
    """
