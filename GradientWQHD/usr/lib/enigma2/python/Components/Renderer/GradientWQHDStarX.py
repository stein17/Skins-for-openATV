...
# -*- coding: utf-8 -*-
# GradientWQHDStarX - patched for stable display in Player + Live
# Based on user's "OPTIMIERTE GradientWQHDStarX.py - Version 3" (minimal functional changes)
...
class GradientWQHDStarX(Renderer):
    GUI_WIDGET = ePixmap
    ...
    def _queue_worker(self, request):
        ...
        if start_worker:
            worker = threading.Thread(target=self._worker_loop, name='GradientWQHDStarX-worker')
            worker.daemon = True
            worker.start()
        try:
