...
"""
OPTIMIERTE GradientWQHDPosterXEMC.py
...
3. JSON-Speicherung fuer Rating/FSK (GradientWQHDStarX/GradientWQHDParental)
4. Robustere Fehlerbehandlung
5. Bessere Dateinamensbereinigung

SKIN.XML BEISPIELE:
<!-- EMC Selection -->
<widget source="Service" render="GradientWQHDPosterXEMC" position="825,506" size="110,164" zPosition="4" alphatest="blend" />

<!-- Movie Player -->
<widget source="session.CurrentService" render="GradientWQHDPosterXEMC" position="10,325" size="170,255" zPosition="4" alphatest="blend" />

<!-- Movieliste -->
<widget source="ServiceEvent" render="GradientWQHDPosterXEMC" position="750,385" size="170,255" zPosition="4" alphatest="blend" />
"""
...
try:
    from .GradientWQHDConverlibr import convtext, cutName
except:
    from GradientWQHDConverlibr import convtext, cutName
...
def _log_active_paths_once():
    try:
        with open(PATHS_LOG, "a+", encoding="utf-8") as lf:
            lf.write("[%s] GradientWQHDPosterXEMC EMC_BASE=%s\n" % (time.strftime("%Y-%m-%d %H:%M:%S"), EMC_BASE))
    except Exception:
        pass
...
def save_info_json(slug, data):
    """Speichert Info-JSON fuer GradientWQHDStarX/GradientWQHDParental"""
...
class GradientWQHDPosterXEMC(Renderer):
    """
...
if __name__ == '__main__':
    print("=" * 60)
    print("GradientWQHDPosterXEMC - OPTIMIERTE VERSION")
    print("=" * 60)
    print()
    print("FEATURES:")
