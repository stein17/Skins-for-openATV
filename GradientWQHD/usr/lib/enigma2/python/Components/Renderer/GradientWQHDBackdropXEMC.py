# OPTIMIERTE VERSION - GradientWQHDBackdropXEMC.py
...
"""
2. NEUE GradientWQHDBackdropXEMC.py
================================

Fuer Backdrop bei Aufnahmen/Filmen (EMC, Movie Player, Movieliste)

FEATURES:
- Gleiche Logik wie GradientWQHDPosterXEMC.py
- TITLE_MAPPINGS Integration
...
SKIN.XML BEISPIELE:
<!-- EMC Selection -->
<widget source="Service" render="GradientWQHDBackdropXEMC" position="10,100" size="600,337" zPosition="3" alphatest="blend" />

<!-- Movie Player -->
<widget source="session.CurrentService" render="GradientWQHDBackdropXEMC" position="0,0" size="1280,720" zPosition="1" alphatest="blend" />
"""
...
try:
    from .GradientWQHDConverlibr import convtext, cutName, apply_title_mapping
except:
    from GradientWQHDConverlibr import convtext, cutName, apply_title_mapping
DEBUG_EMC = False
...
def _log_active_paths_once():
    try:
        with open(PATHS_LOG, "a+", encoding="utf-8") as lf:
            lf.write("[%s] GradientWQHDBackdropXEMC EMC_BASE=%s\n" % (time.strftime("%Y-%m-%d %H:%M:%S"), EMC_BASE))
    except Exception:
        pass
...
class GradientWQHDBackdropXEMC(Renderer):
    GUI_WIDGET = ePixmap
...
if __name__ == '__main__':
    print('=' * 60)
    print('GradientWQHDBackdropXEMC - NEU')
    print('=' * 60)
    print()
