...
"""GradientWQHDParental.py (OpenATV / Enigma2) - Fix for EMC/MoviePlayer + MovieList
...
EMC MoviePlayer/Playback (best):
  <widget source="session.CurrentService" render="GradientWQHDParental" ... />

EMC MovieList selection (best):
  <widget source="Service" render="GradientWQHDParental" ... />
...
debug="0|1" logfile="/tmp/GradientWQHDParental.log"

Installation
------------
/usr/lib/enigma2/python/Components/Renderer/GradientWQHDParental.py
rm -f /usr/lib/enigma2/python/Components/Renderer/GradientWQHDParental.pyc
...
def default_icon_path():
    try:
        ...
        return '/usr/share/enigma2/GradientWQHD/parental'
...
class GradientWQHDParental(Renderer):
    GUI_WIDGET = ePixmap
    ...
    def __init__(self):
        ...
        self._debug = False
        self._logfile = '/tmp/GradientWQHDParental.log'
    ...
    def _log(self, *parts):
        if not self._debug:
            return
        msg = "[GradientWQHDParental] %s %s" % (time.strftime("%Y-%m-%d %H:%M:%S"), " ".join([str(p) for p in parts]))
        try:
