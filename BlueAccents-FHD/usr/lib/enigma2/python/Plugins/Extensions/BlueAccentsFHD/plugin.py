...
def Plugins(**kwargs):
    return [PluginDescriptor(name=_("BlueAccents FHD Configtool"), description=_("Personalize your BlueAccents FHD (Skin by stein17)"), where = [PluginDescriptor.WHERE_PLUGINMENU],
    icon="plugin.png", fnc=main)]

def main(session, **kwargs):
    if config.skin.primary_skin.value == "BlueAccents-FHD/skin.xml":
        session.open(BlueAccents_Config)
    else:
        AddPopup(_('Please activate BlueAccents FHD Skin before run the Config Plugin'), type=MessageBox.TYPE_ERROR, timeout=10)
        return []
...
class BlueAccents_Config(Screen, ConfigListScreen):
    ...
    def setWeather(self):
        try:
            ...
            self.session.open(MessageBox, _("'weatherplugin' is not installed!, please install the WeatherComponentHandler from SystemPlugins"), MessageBox.TYPE_INFO)
    ...
    def getInitConfig(self):
        ...
        self.title = _("BlueAccentsFHD Setup")
    ...
    def createConfigList(self):
        self.set_background = getConfigListEntry(_("Background transparency:"), self.myAtileHD_background)
