...
#Channel Selection Backdrop
	#<widget source="ServiceEvent" render="BlueABackdropX" nexts="0" position="750,500" delayPic="0" usedImage="backdrop" cornerRadius="6" size="170,96" zPosition="98" />
...
from Components.Renderer.BlueAPosterXDownloadThread import BlueAPosterXDownloadThread
...
class PosterDB(BlueAPosterXDownloadThread):
        def __init__(self):
                BlueAPosterXDownloadThread.__init__(self)
                self.logdbg = None
...
class PosterAutoDB(BlueAPosterXDownloadThread):
        def __init__(self):
                BlueAPosterXDownloadThread.__init__(self)
                self.logdbg = None
        ...
        def logAutoDB(self, logmsg):
                if self.logdbg:
                        w = open(path_folder + "BlueAPosterXBD.log", "a+")
                        w.write("%s\n"%logmsg)
                        w.close()
...
class BlueAPosterX(Renderer):
        ...
        def logPoster(self, logmsg):
                if self.logdbg:
                        w = open(path_folder + "BlueAPosterX.log", "a+")
                        w.write("%s\n"%logmsg)
                        w.close() 
