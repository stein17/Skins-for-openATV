...
class SkinManager(object):
    ...
    def _migrate_directory(self, source):
        ...
        for name in names:
            ...
            try:
                ...
                print("[BundesligaWQHDConfig] Migration failed for %s: %s" % (src, error))
    ...
    def _replace_directory_with_link(path_value, target):
        if os.path.islink(path_value):
            ...
            try:
                ...
                print("[BundesligaWQHDConfig] Could not remove legacy directory %s: %s" % (path_value, error))
                return
        ...
        try:
            ...
            print("[BundesligaWQHDConfig] Could not create link %s: %s" % (path_value, error))
    ...
    def team_uses_special_config_style(self, source=None):
        ...
        try:
            ...
            print("[BundesligaWQHDConfig] Team profile XML check failed: %s" % error)
        ...
        try:
            ...
            print("[BundesligaWQHDConfig] Team profile fallback check failed: %s" % error)
            return False
    ...
    def apply_team(self, source):
        ...
        # The skin always uses Verein/select_72.png.  Only this link changes,
