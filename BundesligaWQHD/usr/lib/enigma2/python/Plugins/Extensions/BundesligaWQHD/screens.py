...
<screen name="BundesligaWQHDConfig" position="0,0" size="2560,1440" title="BundesligaWQHD Config" flags="wfNoBorder" backgroundColor="transparent">
    ...
    <panel position="40,33" size="173,173">
...
def _set_preview(screen, preview, scale=False):
    ...
    if preview:
        try:
            ...
            print("[BundesligaWQHDConfig] Preview error: %s" % error)

    picture.hide()
    return False


class BundesligaWQHDTeamDownload(Screen):
    ...
    def _start(self):
        ...
        try:
            ...
            return
        worker = Thread(target=self._worker, name="BundesligaWQHD-TeamDownload")
        worker.daemon = True
        worker.start()
...
class BundesligaWQHDWeatherIconsetDownload(Screen):
    skin = TEAM_DOWNLOAD_SKIN
    ...
    def _start(self):
        ...
        try:
            ...
            self.close({"ok": False, "error": _("Downloaddienst nicht verfügbar: %s") % error})
            return
        worker = Thread(target=self._worker, name="BundesligaWQHD-WeatherIconsetDownload")
        worker.daemon = True
        worker.start()
...
class BundesligaWQHDTeamManager(Screen):
...
class BundesligaWQHDConfig(Screen, ConfigListScreen):
    ...
    def openSkinparts(self):
        if not self.manager.discover_categories():
            ...
            return
        self.session.openWithCallback(self.skinpartsCallback, BundesligaWQHDSkinParts, self.manager)
    ...
    def openTeamManager(self):
        self.session.open(BundesligaWQHDTeamManager, self.manager)
    ...
    def _iconset_download_answer(self, answer, team_changed, entry):
        ...
        self.session.openWithCallback(
            ...
            BundesligaWQHDWeatherIconsetDownload,
            self.weather_icons,
            entry
        )
...
class BundesligaWQHDSkinParts(Screen, ConfigListScreen):
