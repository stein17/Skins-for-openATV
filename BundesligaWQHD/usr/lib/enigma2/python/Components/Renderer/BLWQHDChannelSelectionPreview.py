...
class BLWQHDChannelSelectionPreview(Renderer):
	"""Display a preview only for compatible Bundesliga screen/list pairs."""

	GUI_WIDGET = ePixmap

	def __init__(self):
		Renderer.__init__(self)
		self.previewPath = "/usr/share/enigma2/BundesligaWQHD/preview/channelselection"
		self.templateFile = "/usr/share/enigma2/BundesligaWQHD/skinTemplates.xml"
		self.interval = 250
	...
	def _loadTemplates(self, force=False):
		...
		try:
			...
			print("[BLWQHDChannelSelectionPreview] Unable to read templates: %s" % error)
			self.templates = {}
	...
	def _update(self):
		...
		if preview != self.currentPreview:
			try:
				...
				print("[BLWQHDChannelSelectionPreview] Unable to load preview '%s': %s" % (preview, error))
				self._hidePreview()
				return
