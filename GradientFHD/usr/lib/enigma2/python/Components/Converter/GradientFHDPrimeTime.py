...
class GradientFHDPrimeTime(Converter, object):
    """GradientFHDPrimeTime

    Skin examples:
        <convert type="GradientFHDPrimeTime">PrimeTime,noDuration</convert>
        <convert type="GradientFHDPrimeTime">Event2,withDuration</convert>
    ...
    def _getPrimeTimeHM(self):
        """Read PrimeTime hour/minute from config.plugins.GradientFHD.*
        ...
        try:
            sec = getattr(config.plugins, 'GradientFHD', None)
            if sec is not None:
