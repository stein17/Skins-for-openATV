...
"""GradientFHDParental.py (OpenATV / Enigma2) - Fix for EMC/MoviePlayer + MovieList
...
EMC MoviePlayer/Playback (best):
  <widget source="session.CurrentService" render="GradientFHDParental" ... />

EMC MovieList selection (best):
  <widget source="Service" render="GradientFHDParental" ... />
...
debug="0|1" logfile="/tmp/GradientFHDParental.log"

Installation
------------
/usr/lib/enigma2/python/Components/Renderer/GradientFHDParental.py
rm -f /usr/lib/enigma2/python/Components/Renderer/GradientFHDParental.pyc
...
try:
    from .GradientFHDConverlibr import convtext, apply_title_mapping
except Exception:
    try:
        from GradientFHDConverlibr import convtext, apply_title_mapping
    except Exception:
...
class GradientFHDParental(Renderer):
    GUI_WIDGET = ePixmap
    ...
    def __init__(self):
        ...
        self._debug = False
        self._logfile = '/tmp/GradientFHDParental.log'
    ...
    def _log(self, *parts):
        if not self._debug:
            return
        msg = "[GradientFHDParental] %s %s" % (time.strftime("%Y-%m-%d %H:%M:%S"), " ".join([str(p) for p in parts]))
        try:
