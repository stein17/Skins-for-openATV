...
# -*- coding: utf-8 -*-
# GradientFHDStarX - patched for stable display in Player + Live
# Based on user's "OPTIMIERTE GradientFHDStarX.py - Version 3" (minimal functional changes)
...
try:
    from .GradientFHDConverlibr import convtext, quoteEventName, apply_title_mapping
except Exception:
    from GradientFHDConverlibr import convtext, quoteEventName, apply_title_mapping
...
class GradientFHDStarX(Renderer):
    GUI_WIDGET = ePixmap
    ...
    def _queue_worker(self, request):
        ...
        if start_worker:
            worker = threading.Thread(target=self._worker_loop, name='GradientFHDStarX-worker')
            worker.daemon = True
            worker.start()
        try:
