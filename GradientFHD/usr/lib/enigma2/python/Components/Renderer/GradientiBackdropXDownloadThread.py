...
from .GradientiConverlibr import quoteEventName
...
try:
    ...
    print("Error loading the data API:", str(e))
    my_cur_skin = False
...
class GradientiBackdropXDownloadThread(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.adsl = intCheck()
        if not self.adsl:
            print("No connection, offline mode.")
            return
        else:
    ...
    def search_omdb(self, dwn_backdrop, title, shortdesc, fulldesc, channel=None):
        try:
            ...
            if response.status_code == requests.codes.ok:
                ...
                return False, f"Error during the search OMDB: {response.status_code}"
        except Exception as e:
            print('Error in search OMDB:', e)
            return False, "Error during the search OMDB"
    ...
    def run(self):
        ...
        while True:
            ...
            if self.pstcanal is not None:
                ...
                pdb.task_done()  # To avoid blocking threads
                continue
