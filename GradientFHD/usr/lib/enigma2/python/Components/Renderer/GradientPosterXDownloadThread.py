...
class GradientPosterXDownloadThread(threading.Thread):
    def __init__(self):
        ...
        # event info helper instance (one per thread object)
        try:
    ...
    def search_tmdb(self,dwn_poster,title,shortdesc,fulldesc,channel=None):
        try:
            ...
            # Save info JSON via Gradient_event_info (merge) if helper is available
            if hasattr(self, "_event_info") and self._event_info:
                try:
                    # build a safe slug from title
                    slug_local = re.sub(r'[^\w\s-]', '', title).strip().lower()
                    slug_local = re.sub(r'\s+', '_', slug_local)
                except Exception:
                    slug_local = None
                try:
                    first = poster['results'][0] if poster and poster.get('results') else None
                    if first and slug_local:
                        # fetch details from TMDb and save into INFO_FOLDER under slug_local.json
                        self._event_info.fetch_and_save(title, slug_local)
                except Exception:
                    pass
