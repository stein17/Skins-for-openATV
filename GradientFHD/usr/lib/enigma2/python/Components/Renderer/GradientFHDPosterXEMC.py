...
"""
OPTIMIERTE GradientFHDPosterXEMC.py
...
3. JSON-Speicherung fuer Rating/FSK (GradientFHDStarX/GradientFHDParental)
4. Robustere Fehlerbehandlung
5. Bessere Dateinamensbereinigung

SKIN.XML BEISPIELE:
<!-- EMC Selection -->
<widget source="Service" render="GradientFHDPosterXEMC" position="825,506" size="110,164" zPosition="4" alphatest="blend" />

<!-- Movie Player -->
<widget source="session.CurrentService" render="GradientFHDPosterXEMC" position="10,325" size="170,255" zPosition="4" alphatest="blend" />

<!-- Movieliste -->
<widget source="ServiceEvent" render="GradientFHDPosterXEMC" position="750,385" size="170,255" zPosition="4" alphatest="blend" />
"""
...
try:
    from .GradientFHDConverlibr import convtext, cutName
except:
    from GradientFHDConverlibr import convtext, cutName
...
def _log_active_paths_once():
    try:
        with open(PATHS_LOG, "a+", encoding="utf-8") as lf:
            lf.write("[%s] GradientFHDPosterXEMC EMC_BASE=%s\n" % (time.strftime("%Y-%m-%d %H:%M:%S"), EMC_BASE))
    except Exception:
        pass
...
# TITLE_MAPPINGS - Gleiche wie in GradientFHDPosterXDownloadThread
TITLE_MAPPINGS = {
    # Daily Soaps
...
def save_info_json(slug, data):
    """Speichert Info-JSON fuer GradientFHDStarX/GradientFHDParental"""
...
class GradientFHDPosterXEMC(Renderer):
    """
...
if __name__ == '__main__':
    print("=" * 60)
    print("GradientFHDPosterXEMC - OPTIMIERTE VERSION")
    print("=" * 60)
    print()
    print("FEATURES:")
