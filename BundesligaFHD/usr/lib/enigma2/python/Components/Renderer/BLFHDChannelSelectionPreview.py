...
class BLFHDChannelSelectionPreview(Renderer):
	...
	def __init__(self):
		...
		self.previewPath = "/usr/share/enigma2/BundesligaFHD/preview/channelselection"
		self.templateFile = "/usr/share/enigma2/BundesligaFHD/skinTemplates.xml"
		self.interval = 250
	...
	def _loadTemplates(self, force=False):
		...
		try:
			...
			print("[BLFHDChannelSelectionPreview] Unable to read templates: %s" % error)
			self.templates = {}
	...
	def _update(self):
		...
		if preview != self.currentPreview:
			try:
				...
				print("[BLFHDChannelSelectionPreview] Unable to load preview '%s': %s" % (preview, error))
				self._hidePreview()
				return
